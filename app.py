...
controls = dbc.Card(
    [
        html.Div(
            [
                dbc.Label(
                    "The first indicator (global) / Diet component (Country specific)"
                ),
                dcc.Dropdown(
                    id="x-variable",
                    options=[{"label": f"{col})", "value": col} for col in indicator],
        ...
        html.Div(
            [
                dbc.Label(
                    "The second indicator (global) / Disabled (Country specific)"
                ),
                dcc.Dropdown(
                    id="y-variable",
                    options=[{"label": col, "value": col} for col in indicator],
...
app.layout = html.Div(
    children=[
        dbc.Container(
            dbc.Row(
                [
                    ...
                    html.H4(
                        children="This visualizations shows different dietary or social factors and their interaction with "
                        "COVID-19 mortality."
                    ),
                ],
                justify="center",
            ...
            children=[
                ...
                html.Div(
                    children=dbc.RadioItems(
                        id="main_map_radios",
                        options=[
                            {"label": col.capitalize().replace("_", " "), "value": col}
        ...
        html.Div(
            ...
            style={"vertical-align": "top", "width": "40%", "display": "inline-block"},
        ),
        html.Div(
            dbc.Container(
                [
            ...
            style={"vertical-align": "top", "width": "60%", "display": "inline-block"},
        ),
        dbc.Row(
            dbc.Col(
                html.Div(
...
def update_h1(main_map, n_clicks):
    if (
        main_map
        and not dash.callback_context.triggered[0]["prop_id"].split(".")[0] == "world"
    ):
...
def update_dropdown_variable(main_map, n_clicks):
    if (
        ...
        return [{"label": i, "value": i} for i in indicator], [
            {"label": i, "value": i} for i in indicator
        ]
...
def update_fig2(x_variable, y_variable, main_map):
    ...
    if x_variable == "Food Source":
        ...
        return px.pie(
            src, values="Percentages", names="Items", title=f"{x_variable} in {country}"
        )
        ...
        return px.scatter(
            df, x=x_variable, y=y_variable, hover_name="Country", title=""
        )
...
def update_time_series(main_map, n_clicks):
    ...
    if (
        ...
        iso_ts = (
            time_series.groupby("Date")
            .agg({"New Cases Per Million": "sum", "New Deaths Per Million": "sum"})
            .reset_index()
        )
    fig = px.bar(
        iso_ts,
        x="Date",
        y=["New Cases Per Million", "New Deaths Per Million"],
        barmode="overlay",
        title=f"New cases and mortality caused by COVID-19 trends in the {country}",
    )
    fig.update_layout(
        legend_title_text=f"Evolution of COVID-19 pandemic in the {country}"
    )
    fig.layout.xaxis.dtick = "M1"
    return fig


@app.callback(Output("main_map", "figure"), Input("main_map_radios", "value"))
